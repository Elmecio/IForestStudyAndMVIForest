...
class IsolationForest:
    ...
    def anomaly_score_with_details(self, X:np.ndarray) -> np.ndarray:
        """
        To get anomaly score and data path length for every trees
        """
...
class IsolationTree:
    ...
    def fit(self, X:np.ndarray):
        ...
        if min_x == max_x:
            ...
            #Use the mean of data in the node instead of a random value
            split_value = X_col.mean()
            print("min = "+str(min_x)+" max = "+str(max_x)+ " mean = "+str(split_value))
